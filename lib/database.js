...
async function checkRepoAvailability() {
  try {
    const apiUrl = `https://api.github.com/repos/VajiraTech/QUEEN-IZUMI-MD`;
const headers = {
...
async function githubSearchFile(filePath, fileName) {
  const url = `https://api.github.com/repos/VajiraTech/QUEEN-IZUMI-MD/config.js?ref=main`;
...
async function githubCreateNewFile(filePath, fileName, content) {
  const url = `https://api.github.com/repos/VajiraTech/QUEEN-IZUMI-MD/config.js`;
  const data = {
...
async function githubDeleteFile(filePath, fileName) {
  const file = await githubSearchFile('config.js');
...
await githubCreateNewFile("./config.js",js.stringify(get))
console.log(`Database "${repoName}" created successfully 🛢️`);
}
else console.log("Database connected 🛢️")
};
//=====================================================================
async function input(config, data){
let get = js.parse(await githubGetFileContent("./config.js"))
 
if (setting == "LANG") {
get.LANG = data
config.LANG = data
return await githubClearAndWriteFile("./config.js",JSON.stringify(get))
} else if (setting == "ANTI_BAD") {
get.ANTI_BAD = data
config.ANTI_BAD = data
return await githubClearAndWriteFile("./config.js",js.stringify(get))
...
async function updb(){
let get = js.parse(await githubGetFileContent("./config"))
...
await githubClearAndWriteFile("./config.js",js.stringify(get))
config.LANG = 'EN'
config.MAX_SIZE = 100
